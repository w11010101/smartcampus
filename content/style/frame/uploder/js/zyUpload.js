		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info">上传图片，您最多能选择4张图片</div>';
		            html += '				<div class="btns">';
				    ...
				    html += '							<img class="upload_image" src="../../content/style/frame/uploder/images/img-add.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				var imgnumber_aa = 4-num;
				
				if(imgnumber_aa<1){
					$(".add_upload").css("display","none");
					$("#status_info").html("您还可以添加0张图片。");
					return;
				}else{
					$("#status_info").html("您还可以添加"+imgnumber_aa+"张图片。");
					setTimeout(function(){
						$(".add_upload").css("display","block");
					},500)
				}
				return;
			};
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funAppendPreviewHtml = function(html){
							...
							if(para.dragDrop){
								...
								console.log($("#preview>.upload_append_list").length);
								if($("#preview>.upload_append_list").length>4){
									$("#preview>.upload_append_list:gt(3)").remove();
								}
							}
							// 绑定删除按钮
							funBindDelEvent();
